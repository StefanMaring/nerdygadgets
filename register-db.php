...
if($_POST["password"] != $_POST["repeat_password"]){
    $_SESSION["user_notice_message"] = array("Wachtwoord is niet gelijk!");
    ...
    if (!empty($voornaam) && !empty($achternaam) && !empty($email) && !empty($tel) && !empty($adres) && !empty($postcode) && !empty($woonplaats) && !empty($plaintext_password)) {
        ...
        setUser($customerID);   //Log registreerde gebruiker gelijk in
        header("location: index.php");  //Stuur terug naar hoofdpagina
        exit();
    } else {
